...
├── assets/           # Bundled images (pb_logo.png for EPUB dividers/end page)
├── cache.py          # In-memory content cache (ContentCache, CacheStats)
├── cli.py            # Click CLI entry point (build, deliver commands)
├── config.py         # YAML config loading + validation (Config, FeedConfig w/ category + frequency metadata, DeliveryConfig, EmailConfig)
...
feeds:
  - name: "The Guardian"
    ...
    articles_per_day: 12.0       # Optional — from feed_stats, controls freshness window
    estimated_read_min: 3.5      # Optional — from feed_stats, controls scarce persistence
delivery:
...
- **`apply_reading_time_budget(sections, target_minutes)`** — word count / 238 WPM. Scarcity-aware: when `Section.articles_per_day` is set (from `feed_stats`), uses real frequency for scarcity ranking; otherwise approximates from article count / 20. Scarce sources get priority in both phases. 5-minute overshoot cap prevents awkward cuts while keeping the budget soft.
...
- **Cover layout**: Newspaper broadsheet style — double-rule masthead with auto-scaling title, red-accented lead headline, secondary headlines grouped by source (one per source, with category labels). Masthead font scales down automatically for long titles.
- **EPUB structure**: Category-grouped layout when feeds have categories (web app builds), flat layout when no categories (CLI mode). Spine order: cover page (non-linear) → front page TOC → [category divider → [articles]*]* → end page. The source/section name is rendered inline as an em-dash ornament heading ("— AXIOS —") above the first article of each source (no standalone section divider pages). Cover is non-linear so e-readers open to the TOC front page. End page shows a randomly selected sign-off from 5 variations with the Paper Boy logo. EPUB3 landmarks generated via `book.guide` + `epub3_landmark=True` write option (`cover`, `toc`, `bodymatter` all pointing to front page) + EPUB2 guide. Paper Boy logo (`assets/pb_logo.png`) embedded in EPUB, shown on category dividers and end page. TOC uses `epub.Link` (not `epub.Section`) so NCX navPoints have valid `<content src>` attributes.
- **EPUB CSS**: Optimized for e-ink displays (minimum gray #555) — paragraph indent (except first), justified text, larger body font (1.05em), improved line-height (1.7). Avoids CSS Grid/Flexbox (unsupported on most e-readers). Two-level TOC hierarchy: categories (bold uppercase) → articles (avoids 3-level rendering bugs on Apple Books). Front page custom TOC shows three levels visually: categories (1.5em bold uppercase), sources (1em bold italic), articles (indented). Category dividers are centered openers (2.2em, 60% rules, page-break-before, 35% top margin) with linked source names and logo. Section/source names rendered inline above the first article as em-dash ornament (1em, centered uppercase, letter-spaced, no border). Article-embedded `<hr>` tags styled subtle (#ddd, narrow) to avoid confusion with structural elements. Article source shows domain only (`via theguardian.com`). Article dates normalized to human-readable format (`March 7, 2026`).
...
- Multi-strategy article extraction with fallback chain (`_extract_article_content`):
  0. Domain-specific handlers:
     ...
     - Business of Fashion (`_fetch_bof_feed`): feed-level handler. BoF's RSS feed is broken (500 Internal Server Error on Arc Publishing's outboundfeeds endpoint). Uses homepage scraping for article discovery (`/articles/*` links), then parses `Fusion.globalContent` JavaScript object (Arc Publishing platform, same `content_elements` structure as WaPo's `__NEXT_DATA__`). Extracts `image` content_elements alongside text/header/raw_html/list — uses `url` field (falls back to `additional_properties.originalUrl`), `alt_text`, and `caption` to build `<figure>` tags. All articles serve full content regardless of `content_restrictions.content_code` (paywall is client-side JS only). No special UA or auth needed.
...
- **Content filtering pipeline** (`filters.py`): Runs after extraction, before image processing. Five general-purpose filters:
  ...
  - `strip_trailing_junk(html)` — removes trailing metadata (wire bylines, editorial credits). Uses lxml. Rules in `_TRAILING_JUNK_RULES` list (11 rules: Got a tip?, BBC trailing email, AP section links, Reporting/Writing/Compiled by, Edited by, Additional reporting by, AP author bio, Image credits, Space.com author bio, Correction, BoF standalone byline).
...
- **Declarative pattern architecture**: Both `feeds.py` and `filters.py` use declarative rule lists for easy extension:
  ...
  - `_JSON_IMAGE_EXTRACTORS` in `feeds.py` — dict mapping domains to JSON image extraction functions (adding a new site = writing an extractor function + one dict entry)
- **Frequency-aware freshness windows**: `_freshness_window_days(feed_cfg)` computes per-feed staleness thresholds based on `FeedConfig.articles_per_day` (from `feed_stats`). Four tiers: prolific (≥3/day → 1.5 days), moderate (≥0.5/day → 2 days), scarce (<0.5/day → 2–7 days scaled by article length), unknown (0 → 7 days, preserving legacy default). `_is_stale_entry(entry, max_age_days)` uses the computed window. Prevents daily sources from repeating across editions while letting weekly long-form content persist.
- **Stale entry filtering**: `_is_stale_entry()` skips RSS entries older than the per-feed freshness window using feedparser's `published_parsed`/`updated_parsed` UTC time tuples. Runs in `_fetch_single_feed()` and `_extract_ft_articles()` before extraction. Entries with no date are assumed fresh.
- **Image recovery from raw HTML**: `_recover_images_from_html()` recovers images that trafilatura drops during content boundary detection. When extraction produces 0 images, tries three recovery tiers in order: (1) JSON-based extraction via `_recover_images_from_json()` for domains with embedded structured data — `_JSON_IMAGE_EXTRACTORS` dict maps domains to extractor functions (`_extract_verge_images` for The Verge's `__NEXT_DATA__`, `_extract_conde_nast_images` for Wired/New Yorker's `__PRELOADED_STATE__`), capped at `_JSON_IMAGE_CAP` (5) images; (2) domain-specific xpaths via `_DOMAIN_IMAGE_XPATHS` dict (e.g. AP News uses `RichTextStoryBody`/`bsp-figure` to avoid carousel images); (3) generic container xpaths (`<article>`, `<main>`, `<figure>`, etc.). Filters ads/tracking via `_should_skip_image()`, supports lazy-loading (`data-src`, `data-lazy-src`, `data-original`), resolves relative URLs against the page's base URL, deduplicates, and injects recovered images. Hooked into strategies S0b, S1, S1.5, and S2. For S1 (where trafilatura fetches internally), refetches with browser UA only when 0 images detected.
