...
import { DEVICES, DELIVERY_TIMES, TIMEZONES, GOOGLE_DRIVE_DISABLED, defaultDriveFolderForDevice } from "@/lib/constants";
import type {
  CatalogBundle,
  CatalogCategory,
  CatalogFeed,
...
interface OnboardingWizardProps {
  ...
  bundleFeedMap: Record<string, CatalogFeed[]>;
}
...
export function OnboardingWizard({
  ...
  bundles,
  categories,
  bundleFeedMap: bundleFeedMapRecord,
}: OnboardingWizardProps) {
  const router = useRouter();
  const { state, update, goToStep, loaded } = useOnboardingState();

  // Convert Record to Map for efficient lookups
  const bundleFeedMap = useMemo(
    () => new Map(Object.entries(bundleFeedMapRecord)),
    [bundleFeedMapRecord]
  );
  ...
  function navigateToStep(step: number) {
    if (step === 4 && !email && state.recipientEmail) {
            ...
            {methods.map((m) => {
              ...
              return (
                <div key={m.value}>
                  <label
                    className={cn(
                      "newsprint-card flex items-start gap-3 overflow-hidden border border-rule-gray bg-card px-4 py-3",
    ...
    return (
      <div className="space-y-6">
        ...
        >
          ...
          {googleAuthLoading ? <>Redirecting<LoadingDots /></> : "Continue with Google"}
        </Button>
        ...
        {authError && (
          authError === "check_email" ? (
            <div className="newsprint-card overflow-hidden border border-rule-gray border-l-4 border-l-edition-red bg-card px-5 py-4">
              ...
              <p className="mt-3 font-body text-xs text-caption">
                ...
                <button
                  type="button"
                  onClick={() => { setAuthError(null); setEmailAuthLoading(false); }}
