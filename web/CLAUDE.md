...
│   ├── budget-bar.tsx        # Budget bar: reading time stepper, fill bar, pipeline messaging (~N articles → best M picked → Xm paper)
...
│   ├── reading-time.ts # Reading time helpers: getFrequencyLabel(), formatDailyReadTime(), totalSourceDailyOutput(), totalDailyArticles(), avgEstimatedReadMin(), formatChipReadTime(), hasAnyStats()
